...
export const NavbarMyDetailAssignmentEmptyComponent = () => {

    const navigate = useNavigate();
    const location = useLocation();
    const { slug , class_id , id } = useParams();

    const isDetailClassActive = location.pathname === `/view/${slug}/${class_id}/detail/assignment/${id}`
    const isStudentActive = location.pathname === `/view/${slug}/${class_id}/detail/assignment/${id}/students`
    ...
    return(
        <>
            <nav className="w-full fixed  bg-white" style={{  zIndex:"44" , minWidth:"333px" , maxWidth:"1500px", borderBottom:"1px solid #E5E3E9"}}>
                <header className="sm:w-10/12 w-11/12 py-2  mx-auto">
                    <div className="flex  justify-between gap-4" style={{maxWidth:"1500px",}}>
                                    ...
                                    <div style={{ fontSize:"16px"}} className="font-medium font16-res-300 mt-1">
                                        <ul className="list-none gap-6 flex" style={{ fontWeight :"500"}}>
                                            <li className="pe-6 my-auto" style={{ borderRight:"1px solid #ebebeb"}}>
                                                <Link  style={{ fontWeight:"500"}} className={` ${isDetailClassActive ? 'text-purple-600' : 'text-gray-400'}  my-0 relative cursor-pointer hover:text-purple-600 font`} to={`/view/${slug}/${class_id}/my/absent/${id}`}>Tugas
                                            ...
                                            <li className="pe-6 my-auto" style={{ borderRight:"1px solid #ebebeb"}}>
                                                <Link style={{ fontWeight:"500"}}  className={` ${isStudentActive ? 'text-purple-600' : 'text-gray-400'}  my-0 relative cursor-pointer hover:text-purple-600 font`} to={`/view/${slug}/${class_id}/my/absent/${id}/students`}>Murid
                                                    <div className="w-full mx-auto  absolute top-2 my-3  h-1 cursor-pointer hover:bg-purple-400  block hover:scale-x-50 transform origin-center  transition-transform duration-300">
                                                    </div>
                                                </Link>
                                            </li>
                        ...
                        <div className="lg:hidden lg:w-full relative flex items-center w-auto sm:flex sm:items-center sm:w-auto md:flex md:items-center md:w-auto">
                            ...
                            >
                                <li className="pt-8 pb-4">
                                    <Link  className={` ${isDetailClassActive ? 'text-purple-600' : 'text-gray-400'}   text-purple-400  cursor-pointer hover:text-purple-600`} to={`/view/${slug}/${class_id}/my/absent/${id}`}>Tugas
                                ...
                                <li className="py-4">
                                    <Link  className={`   ${isStudentActive ? 'text-purple-600' : 'text-gray-400'}  cursor-pointer hover:text-purple-600`}  to={`/view/${slug}/${class_id}/my/absent/${id}/students`}>Murid
                                        <div className="w-1/12 mx-auto h-1 cursor-pointer hover:bg-purple-400  block hover:scale-x-50 transform origin-center  transition-transform duration-300">
                                        </div>
                                    </Link>
                                </li>
